...
tam=len(dict_asignaturas[1, 1, "ES"])+len(dict_asignaturas[1, 1, "EU"])\
    #+len(dict_asignaturas[2, 1, "ES"])+len(dict_asignaturas[2, 1, "EU"])+len(dict_asignaturas[2, 1, "IN"])+len(dict_asignaturas[3, 1, "ES"])+len(dict_asignaturas[3, 1, "EU"])
ind_tam=tam*20
asignaturas = dict_asignaturas[1, 1, "ES"]+dict_asignaturas[1, 1, "EU"]\
              #+dict_asignaturas[2, 1, "ES"]+dict_asignaturas[2, 1, "EU"]+dict_asignaturas[2, 1, "IN"]+dict_asignaturas[3, 1, "ES"]+dict_asignaturas[3, 1, "EU"]
...
df_result_3= pd.DataFrame({"Valor fitness_3":vector_result[2000000:pop_tam]})

filename="C:\\Users\\tr5568\\OneDrive - Axalta\\Desktop\\DAYANA\\PERSONAL\\" \
                        "MÁSTER INGENIERÍA COMPUTACIONAL Y SISTEMAS INTELIGENTES\\TFM\\RESULTADOS\\Resultados_baseline_11.xlsx"
